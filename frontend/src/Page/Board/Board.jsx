      ...
      <div className='overflow-x-auto'>
        <table className='min-w-full bg-white border rounded-lg'>
          <thead className='bg-gray-50'>
            <tr>
              <th className='px-6 py-3 text-center text-sm font-medium text-gray-500 uppercase tracking-wider w-[8%]'>번호</th>
              <th className='px-6 py-3 text-center text-sm font-medium text-gray-500 uppercase tracking-wider w-auto'>제목</th>
              <th className='px-6 py-3 text-center text-sm font-medium text-gray-500 uppercase tracking-wider w-[15%]'>작성일</th>
              <th className='px-6 py-3 text-center text-sm font-medium text-gray-500 uppercase tracking-wider w-[8%]'>조회수</th>
            </tr>
          </thead>
          <tbody className='divide-y divide-gray-200'>
            {currentPosts.map((post) =>(
              <tr key={post._id} className="hover:bg-gray-50 cursor-pointer">
                <td className="text-center px-6 py-4 whitespace-nowrap">{post.number}</td>
                <td className="px-6 py-4 whitespace-nowrap">{post.title}</td>
                <td className="text-center px-6 py-4 whitespace-nowrap">{post.createdAt}</td>
                <td className="text-center px-6 py-4 whitespace-nowrap">{post.views}</td>
              </tr>
            ))}
          </tbody>
        </table>
